<?php
include("template-parts/header.php");
include("template-parts/breadcrumbs.php");
?>
    <div class="slot-article">
        <section class="single-slot-article container-wide">
            <h1 class="single-slot-article__title">
                <span class="single-slot-article__icon">
                    <picture>
                        <source srcset="images/img-test/102x60_h1.jpg" media="(max-width: 600px)" type="image/webp">
                        <img src="images/img-test/102x60_h1.jpg" alt="" class="lazy">
                    </picture>
                </span>
                Dazzle Me Megaways Slot Review
            </h1>
            <div class="single-slot-article__container">
                <div class="single-slot-article__img">
                    <div class="single-slot-article__items">
                        <div class="single-slot-article__item">
                            <picture>
                                <source srcset="images/img-test/920x474.jpg" media="(max-width: 600px)" type="image/webp">
                                <img src="images/img-test/920x474.jpg" alt="" class="lazy">
                            </picture>
                        </div>
                        <div class="single-slot-article__item">
                            <picture>
                                <source srcset="images/img-test/920x474.jpg" media="(max-width: 600px)" type="image/webp">
                                <img src="images/img-test/920x474.jpg" alt="" class="lazy">
                            </picture>
                        </div>
                    </div>
                    <div class="slick-slider-dots"></div>
                </div>
                <div class="single-slot-article__desc">
                    <div class="single-slot-article__desc-img">
                        <picture>
                            <source srcset="images/img-test/369x243.jpg" media="(max-width: 600px)" type="image/webp">
                            <img src="images/img-test/369x243.jpg" alt="" class="lazy">
                        </picture>
                    </div>
                    <div class="single-slot-article__desc-tag">
                        <a href="#" class="single-slot-article__desc-lnk" style="background-color: #00DB5C;">Fantastic graphic</a>
                        <a href="#" class="single-slot-article__desc-lnk" style="background-color: #D61B23;">Fantastic graphic</a>
                        <a href="#" class="single-slot-article__desc-lnk" style="background-color: #2D9CDB;">Fantastic graphic</a>
                        <a href="#" class="single-slot-article__desc-lnk" style="background-color: #9B51E0;">Fantastic graphic</a>
                    </div>
                    <div class="rate-block">
                        <h2 class="rate-block__title">
                            Dazzle Me Megaways Rating
                        </h2>
                        <div class="rate-block__container">
                            <div class="rate-block__digit" style="background-color: #FF2F38"> <!-- цвет можно сразу вшить red-#FF2F38 green-#00DB5C orange-#F4B91F -->
                                <div class="rate-block__num">
                                    2.5
                                </div>
                                <div class="rate-block__voutes">
                                    110 votes
                                </div>
                            </div>
                            <div class="rate-block__rating">
                                <span>
                                    Rate slot
                                </span>
                                <div class="rate-block__rating-result">
                                    <span class="active"></span>
                                    <span class="active"></span>
                                    <span class="active"></span>
                                    <span class="active"></span>
                                    <span></span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="single-slot-article__content">
                    <h2 class="single-slot-article__subtitle">
                        Dazzle Me Megaways Specification
                    </h2>
                    <div class="single-slot-article__tables">
                        <table>
                            <tbody>
                                <tr>
                                    <td class="desc">Release</td>
                                    <td class="value">May <a href="#">2021</a></td>
                                </tr>
                                <tr>
                                    <td class="desc">Provider</td>
                                    <td class="value">NetEnt</td>
                                </tr>
                                <tr>
                                    <td class="desc">Max. RTP</td>
                                    <td class="value">---</td>
                                </tr>
                            </tbody>
                        </table>
                        <table>
                            <tbody>
                            <tr>
                                <td class="desc">Reels</td>
                                <td class="value">6</td>
                            </tr>
                            <tr>
                                <td class="desc">Paylines/Ways</td>
                                <td class="value">117,649</td>
                            </tr>
                            <tr>
                                <td class="desc">Betting Range (€/$)</td>
                                <td class="value">---</td>
                            </tr>
                            </tbody>
                        </table>
                        <table>
                            <tbody>
                            <tr>
                                <td class="desc">Max. Payout</td>
                                <td class="value">---</td>
                            </tr>
                            <tr>
                                <td class="desc">Progressive Jackpot</td>
                                <td class="value">No</td>
                            </tr>
                            <tr>
                                <td class="desc">Megaways</td>
                                <td class="value">Yes</td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </section>
        <section class="best-casinos" style="background-color: #DFF7FF">
            <div class="best-casinos__container container-wide">
                <h2 class="best-casinos__title">Best Playson Online Casinos</h2>
                <div class="best-casinos__content">
                    <div class="best-casinos__item">
                        <div class="best-casinos__img">
                            <img src="images/img-test/304x201.jpg" alt="test">
                            <div class="place">1</div>
                            <div class="rate">
                                <svg width="18" height="17" viewBox="0 0 18 17" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16.7363 5.89649L11.7774 5.17579L9.56055 0.681648C9.50001 0.558601 9.4004 0.458992 9.27735 0.398445C8.96876 0.246101 8.59376 0.373054 8.43946 0.681648L6.22266 5.17579L1.26368 5.89649C1.12696 5.91602 1.00196 5.98047 0.906256 6.07813C0.790556 6.19705 0.7268 6.35704 0.728998 6.52294C0.731195 6.68884 0.799166 6.84708 0.917975 6.9629L4.50587 10.4609L3.65821 15.4004C3.63833 15.5153 3.65105 15.6335 3.69491 15.7415C3.73878 15.8496 3.81204 15.9431 3.90639 16.0117C4.00074 16.0802 4.1124 16.1209 4.22872 16.1292C4.34503 16.1375 4.46134 16.113 4.56446 16.0586L9.00001 13.7266L13.4356 16.0586C13.5566 16.1231 13.6973 16.1445 13.832 16.1211C14.1719 16.0625 14.4004 15.7402 14.3418 15.4004L13.4941 10.4609L17.082 6.9629C17.1797 6.86719 17.2441 6.74219 17.2637 6.60547C17.3164 6.26368 17.0781 5.94727 16.7363 5.89649Z"
                                          fill="#F4D21F"/>
                                </svg>
                                4.3
                            </div>
                            <a href="#" class="info">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M11 7H9V5H11V7ZM11 15H9V9H11V15ZM10 0C8.68678 0 7.38641 0.258658 6.17316 0.761205C4.9599 1.26375 3.85751 2.00035 2.92892 2.92893C1.05355 4.8043 -1.52588e-05 7.34784 -1.52588e-05 10C-1.52588e-05 12.6522 1.05355 15.1957 2.92892 17.0711C3.85751 17.9997 4.9599 18.7363 6.17316 19.2388C7.38641 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.68678 19.7414 7.38642 19.2388 6.17317C18.7363 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0Z"
                                          fill="#F4D21F"/>
                                </svg>
                            </a>
                            <div class="country">
                                <img src="images/img-test/russia.svg" alt="Russia">
                                <img src="images/img-test/eng.svg" alt="England">
                            </div>
                        </div>
                        <div class="best-casinos__title-item">
                            100% up to €100 + 150FS
                        </div>
                        <a href="#" class="best-casinos__permalink white-button">
                            Play now
                        </a>
                        <div class="best-casinos__content-item">
                            <span class="age">18+</span>
                            <span class="applies">T&C applies</span>
                            <span class="safe">Play Safe</span>
                        </div>
                    </div>
                    <div class="best-casinos__item">
                        <div class="best-casinos__img">
                            <img src="images/img-test/304x201.jpg" alt="test">
                            <div class="place">1</div>
                            <div class="rate">
                                <svg width="18" height="17" viewBox="0 0 18 17" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16.7363 5.89649L11.7774 5.17579L9.56055 0.681648C9.50001 0.558601 9.4004 0.458992 9.27735 0.398445C8.96876 0.246101 8.59376 0.373054 8.43946 0.681648L6.22266 5.17579L1.26368 5.89649C1.12696 5.91602 1.00196 5.98047 0.906256 6.07813C0.790556 6.19705 0.7268 6.35704 0.728998 6.52294C0.731195 6.68884 0.799166 6.84708 0.917975 6.9629L4.50587 10.4609L3.65821 15.4004C3.63833 15.5153 3.65105 15.6335 3.69491 15.7415C3.73878 15.8496 3.81204 15.9431 3.90639 16.0117C4.00074 16.0802 4.1124 16.1209 4.22872 16.1292C4.34503 16.1375 4.46134 16.113 4.56446 16.0586L9.00001 13.7266L13.4356 16.0586C13.5566 16.1231 13.6973 16.1445 13.832 16.1211C14.1719 16.0625 14.4004 15.7402 14.3418 15.4004L13.4941 10.4609L17.082 6.9629C17.1797 6.86719 17.2441 6.74219 17.2637 6.60547C17.3164 6.26368 17.0781 5.94727 16.7363 5.89649Z"
                                          fill="#F4D21F"/>
                                </svg>
                                4.3
                            </div>
                            <a href="#" class="info">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M11 7H9V5H11V7ZM11 15H9V9H11V15ZM10 0C8.68678 0 7.38641 0.258658 6.17316 0.761205C4.9599 1.26375 3.85751 2.00035 2.92892 2.92893C1.05355 4.8043 -1.52588e-05 7.34784 -1.52588e-05 10C-1.52588e-05 12.6522 1.05355 15.1957 2.92892 17.0711C3.85751 17.9997 4.9599 18.7363 6.17316 19.2388C7.38641 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.68678 19.7414 7.38642 19.2388 6.17317C18.7363 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0Z"
                                          fill="#F4D21F"/>
                                </svg>
                            </a>
                            <div class="country">
                                <img src="images/img-test/russia.svg" alt="Russia">
                                <img src="images/img-test/eng.svg" alt="England">
                            </div>
                        </div>
                        <div class="best-casinos__title-item">
                            100% up to €100 + 150FS
                        </div>
                        <a href="#" class="best-casinos__permalink white-button">
                            Play now
                        </a>
                        <div class="best-casinos__content-item">
                            <span class="age">18+</span>
                            <span class="applies">T&C applies</span>
                            <span class="safe">Play Safe</span>
                        </div>
                    </div>
                    <div class="best-casinos__item">
                        <div class="best-casinos__img">
                            <img src="images/img-test/304x201.jpg" alt="test">
                            <div class="place">1</div>
                            <div class="rate">
                                <svg width="18" height="17" viewBox="0 0 18 17" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16.7363 5.89649L11.7774 5.17579L9.56055 0.681648C9.50001 0.558601 9.4004 0.458992 9.27735 0.398445C8.96876 0.246101 8.59376 0.373054 8.43946 0.681648L6.22266 5.17579L1.26368 5.89649C1.12696 5.91602 1.00196 5.98047 0.906256 6.07813C0.790556 6.19705 0.7268 6.35704 0.728998 6.52294C0.731195 6.68884 0.799166 6.84708 0.917975 6.9629L4.50587 10.4609L3.65821 15.4004C3.63833 15.5153 3.65105 15.6335 3.69491 15.7415C3.73878 15.8496 3.81204 15.9431 3.90639 16.0117C4.00074 16.0802 4.1124 16.1209 4.22872 16.1292C4.34503 16.1375 4.46134 16.113 4.56446 16.0586L9.00001 13.7266L13.4356 16.0586C13.5566 16.1231 13.6973 16.1445 13.832 16.1211C14.1719 16.0625 14.4004 15.7402 14.3418 15.4004L13.4941 10.4609L17.082 6.9629C17.1797 6.86719 17.2441 6.74219 17.2637 6.60547C17.3164 6.26368 17.0781 5.94727 16.7363 5.89649Z"
                                          fill="#F4D21F"/>
                                </svg>
                                4.3
                            </div>
                            <a href="#" class="info">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M11 7H9V5H11V7ZM11 15H9V9H11V15ZM10 0C8.68678 0 7.38641 0.258658 6.17316 0.761205C4.9599 1.26375 3.85751 2.00035 2.92892 2.92893C1.05355 4.8043 -1.52588e-05 7.34784 -1.52588e-05 10C-1.52588e-05 12.6522 1.05355 15.1957 2.92892 17.0711C3.85751 17.9997 4.9599 18.7363 6.17316 19.2388C7.38641 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.68678 19.7414 7.38642 19.2388 6.17317C18.7363 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0Z"
                                          fill="#F4D21F"/>
                                </svg>
                            </a>
                            <div class="country">
                                <img src="images/img-test/russia.svg" alt="Russia">
                                <img src="images/img-test/eng.svg" alt="England">
                            </div>
                        </div>
                        <div class="best-casinos__title-item">
                            100% up to €100 + 150FS
                        </div>
                        <a href="#" class="best-casinos__permalink white-button">
                            Play now
                        </a>
                        <div class="best-casinos__content-item">
                            <span class="age">18+</span>
                            <span class="applies">T&C applies</span>
                            <span class="safe">Play Safe</span>
                        </div>
                    </div>
                    <div class="best-casinos__item">
                        <div class="best-casinos__img">
                            <img src="images/img-test/304x201.jpg" alt="test">
                            <div class="place">1</div>
                            <div class="rate">
                                <svg width="18" height="17" viewBox="0 0 18 17" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16.7363 5.89649L11.7774 5.17579L9.56055 0.681648C9.50001 0.558601 9.4004 0.458992 9.27735 0.398445C8.96876 0.246101 8.59376 0.373054 8.43946 0.681648L6.22266 5.17579L1.26368 5.89649C1.12696 5.91602 1.00196 5.98047 0.906256 6.07813C0.790556 6.19705 0.7268 6.35704 0.728998 6.52294C0.731195 6.68884 0.799166 6.84708 0.917975 6.9629L4.50587 10.4609L3.65821 15.4004C3.63833 15.5153 3.65105 15.6335 3.69491 15.7415C3.73878 15.8496 3.81204 15.9431 3.90639 16.0117C4.00074 16.0802 4.1124 16.1209 4.22872 16.1292C4.34503 16.1375 4.46134 16.113 4.56446 16.0586L9.00001 13.7266L13.4356 16.0586C13.5566 16.1231 13.6973 16.1445 13.832 16.1211C14.1719 16.0625 14.4004 15.7402 14.3418 15.4004L13.4941 10.4609L17.082 6.9629C17.1797 6.86719 17.2441 6.74219 17.2637 6.60547C17.3164 6.26368 17.0781 5.94727 16.7363 5.89649Z"
                                          fill="#F4D21F"/>
                                </svg>
                                4.3
                            </div>
                            <a href="#" class="info">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M11 7H9V5H11V7ZM11 15H9V9H11V15ZM10 0C8.68678 0 7.38641 0.258658 6.17316 0.761205C4.9599 1.26375 3.85751 2.00035 2.92892 2.92893C1.05355 4.8043 -1.52588e-05 7.34784 -1.52588e-05 10C-1.52588e-05 12.6522 1.05355 15.1957 2.92892 17.0711C3.85751 17.9997 4.9599 18.7363 6.17316 19.2388C7.38641 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.68678 19.7414 7.38642 19.2388 6.17317C18.7363 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0Z"
                                          fill="#F4D21F"/>
                                </svg>
                            </a>
                            <div class="country">
                                <img src="images/img-test/russia.svg" alt="Russia">
                                <img src="images/img-test/eng.svg" alt="England">
                            </div>
                        </div>
                        <div class="best-casinos__title-item">
                            100% up to €100 + 150FS
                        </div>
                        <a href="#" class="best-casinos__permalink white-button">
                            Play now
                        </a>
                        <div class="best-casinos__content-item">
                            <span class="age">18+</span>
                            <span class="applies">T&C applies</span>
                            <span class="safe">Play Safe</span>
                        </div>
                    </div>
                    <div class="best-casinos__item">
                        <div class="best-casinos__img">
                            <img src="images/img-test/304x201.jpg" alt="test">
                            <div class="place">1</div>
                            <div class="rate">
                                <svg width="18" height="17" viewBox="0 0 18 17" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16.7363 5.89649L11.7774 5.17579L9.56055 0.681648C9.50001 0.558601 9.4004 0.458992 9.27735 0.398445C8.96876 0.246101 8.59376 0.373054 8.43946 0.681648L6.22266 5.17579L1.26368 5.89649C1.12696 5.91602 1.00196 5.98047 0.906256 6.07813C0.790556 6.19705 0.7268 6.35704 0.728998 6.52294C0.731195 6.68884 0.799166 6.84708 0.917975 6.9629L4.50587 10.4609L3.65821 15.4004C3.63833 15.5153 3.65105 15.6335 3.69491 15.7415C3.73878 15.8496 3.81204 15.9431 3.90639 16.0117C4.00074 16.0802 4.1124 16.1209 4.22872 16.1292C4.34503 16.1375 4.46134 16.113 4.56446 16.0586L9.00001 13.7266L13.4356 16.0586C13.5566 16.1231 13.6973 16.1445 13.832 16.1211C14.1719 16.0625 14.4004 15.7402 14.3418 15.4004L13.4941 10.4609L17.082 6.9629C17.1797 6.86719 17.2441 6.74219 17.2637 6.60547C17.3164 6.26368 17.0781 5.94727 16.7363 5.89649Z"
                                          fill="#F4D21F"/>
                                </svg>
                                4.3
                            </div>
                            <a href="#" class="info">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M11 7H9V5H11V7ZM11 15H9V9H11V15ZM10 0C8.68678 0 7.38641 0.258658 6.17316 0.761205C4.9599 1.26375 3.85751 2.00035 2.92892 2.92893C1.05355 4.8043 -1.52588e-05 7.34784 -1.52588e-05 10C-1.52588e-05 12.6522 1.05355 15.1957 2.92892 17.0711C3.85751 17.9997 4.9599 18.7363 6.17316 19.2388C7.38641 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.68678 19.7414 7.38642 19.2388 6.17317C18.7363 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0Z"
                                          fill="#F4D21F"/>
                                </svg>
                            </a>
                            <div class="country">
                                <img src="images/img-test/russia.svg" alt="Russia">
                                <img src="images/img-test/eng.svg" alt="England">
                            </div>
                        </div>
                        <div class="best-casinos__title-item">
                            100% up to €100 + 150FS
                        </div>
                        <a href="#" class="best-casinos__permalink white-button">
                            Play now
                        </a>
                        <div class="best-casinos__content-item">
                            <span class="age">18+</span>
                            <span class="applies">T&C applies</span>
                            <span class="safe">Play Safe</span>
                        </div>
                    </div>
                </div>
                <a href="#" class="best-casinos__lnk red-btn">
                    Show more
                    <svg width="17" height="8" viewBox="0 0 17 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
                              d="M13.5447 0.716759L16.7267 3.89874C16.922 4.094 16.922 4.41058 16.7267 4.60585L13.5447 7.78783C13.3495 7.98309 13.0329 7.98309 12.8376 7.78783C12.6424 7.59256 12.6424 7.27598 12.8376 7.08072L15.1661 4.75229H0.373169V3.75229H15.1661L12.8376 1.42387C12.6424 1.2286 12.6424 0.912021 12.8376 0.716759C13.0329 0.521497 13.3495 0.521497 13.5447 0.716759Z"
                              fill="#F4D21F"/>
                    </svg>
                </a>
            </div>
        </section>
        <section class="slots-video container-wide">
            <div class="slots-video__text">
                <div class="slots-video__underheader">
                    <p><a href="#">NetEnt</a> continues to work on <a href="#">new Megaways slot machines</a>, now that they’ve gained access to
                        the license via their latest purchase, a developer called Red Tiger. They are planning
                        on launching Dazzle Me Megaways soon, the latest title to use this engine.
                        </p>
                    <p>Dazzle Me Megaways will feature 6 reels with a maximum of 117,649 Megaways active.</p>
                </div>
                <h2 class="slots-video__title">
                    Dazzle Me Megaways Slot Features
                </h2>
                <div class="slots-video__bottomheader">
                    <p>The Megaways system works by giving the player a random number of symbols for each reel.
                        With 6 reels available, and with 2 to 7 symbols randomly picked for each one, you might
                        get only a few hundred ways to win during some spins, or as many as 117,649 in others. As
                        long as 3 or more consecutive reels hold matching symbols, from leftmost column to the right, you will get paid.
                    </p>
                    <p>Dazzle Me Megaways will feature 6 reels with a maximum of 117,649 Megaways active.</p>
                </div>
            </div>
            <div class="slots-video__video">
                <iframe width="100%" height="100%" src="https://www.youtube.com/embed/ujHcGRIXEzU?controls=0" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            </div>
            <div class="slots-video__about">
                <p>Dazzle Me Megaways is not yet available to play, but we will have a full review for it once it is released.</p>
            </div>
        </section>
        <section class="slots-acardeon container-wide">
            <h2 class="slots-acardeon__title">
                Dazzle Me Megaways Slot Machine FAQ
            </h2>
            <div class="slots-acardeon__list">
                <div class="slots-acardeon__item">
                    <div class="slots-acardeon__header">
                        What is a key options of the Dazzle Me Megaways slot?
                    </div>
                    <div class="slots-acardeon__content">
                        Slot developer: NetEnt , 6 reels with 117,649 paylines .
                    </div>
                </div>
                <div class="slots-acardeon__item">
                    <div class="slots-acardeon__header">
                        Where can I play Dazzle Me Megaways?
                    </div>
                    <div class="slots-acardeon__content">
                        NewSlotGames team prepare a rating of online casinos with NetEnt games. Try top recomended licensed NetEnt casinos with Dazzle Me Megaways slot.
                    </div>
                </div>
                <div class="slots-acardeon__item">
                    <div class="slots-acardeon__header">
                        Can I play Dazzle Me Megaways slot on my phone?
                    </div>
                    <div class="slots-acardeon__content">
                        Yes, it's absolutely mobile-friendly!
                    </div>
                </div>
                <div class="slots-acardeon__item">
                    <div class="slots-acardeon__header">
                        Where can I play this slot machine free?
                    </div>
                    <div class="slots-acardeon__content">
                        You should choose one of the recomended online casinos listed here and push "Play Now" button.
                        Any casinos ask you to register account, any casinos allow to play Dazzle Me Megaways free without registration.
                    </div>
                </div>
            </div>
        </section>
        <section class="best-casinos" style="background-color: #DFF7FF">
            <div class="best-casinos__container container-wide">
                <h2 class="best-casinos__title">Best Playson Online Casinos</h2>
                <div class="best-casinos__content">
                    <div class="best-casinos__item">
                        <div class="best-casinos__img">
                            <img src="images/img-test/304x201.jpg" alt="test">
                            <div class="place">1</div>
                            <div class="rate">
                                <svg width="18" height="17" viewBox="0 0 18 17" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16.7363 5.89649L11.7774 5.17579L9.56055 0.681648C9.50001 0.558601 9.4004 0.458992 9.27735 0.398445C8.96876 0.246101 8.59376 0.373054 8.43946 0.681648L6.22266 5.17579L1.26368 5.89649C1.12696 5.91602 1.00196 5.98047 0.906256 6.07813C0.790556 6.19705 0.7268 6.35704 0.728998 6.52294C0.731195 6.68884 0.799166 6.84708 0.917975 6.9629L4.50587 10.4609L3.65821 15.4004C3.63833 15.5153 3.65105 15.6335 3.69491 15.7415C3.73878 15.8496 3.81204 15.9431 3.90639 16.0117C4.00074 16.0802 4.1124 16.1209 4.22872 16.1292C4.34503 16.1375 4.46134 16.113 4.56446 16.0586L9.00001 13.7266L13.4356 16.0586C13.5566 16.1231 13.6973 16.1445 13.832 16.1211C14.1719 16.0625 14.4004 15.7402 14.3418 15.4004L13.4941 10.4609L17.082 6.9629C17.1797 6.86719 17.2441 6.74219 17.2637 6.60547C17.3164 6.26368 17.0781 5.94727 16.7363 5.89649Z"
                                          fill="#F4D21F"/>
                                </svg>
                                4.3
                            </div>
                            <a href="#" class="info">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M11 7H9V5H11V7ZM11 15H9V9H11V15ZM10 0C8.68678 0 7.38641 0.258658 6.17316 0.761205C4.9599 1.26375 3.85751 2.00035 2.92892 2.92893C1.05355 4.8043 -1.52588e-05 7.34784 -1.52588e-05 10C-1.52588e-05 12.6522 1.05355 15.1957 2.92892 17.0711C3.85751 17.9997 4.9599 18.7363 6.17316 19.2388C7.38641 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.68678 19.7414 7.38642 19.2388 6.17317C18.7363 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0Z"
                                          fill="#F4D21F"/>
                                </svg>
                            </a>
                            <div class="country">
                                <img src="images/img-test/russia.svg" alt="Russia">
                                <img src="images/img-test/eng.svg" alt="England">
                            </div>
                        </div>
                        <div class="best-casinos__title-item">
                            100% up to €100 + 150FS
                        </div>
                        <a href="#" class="best-casinos__permalink white-button">
                            Play now
                        </a>
                        <div class="best-casinos__content-item">
                            <span class="age">18+</span>
                            <span class="applies">T&C applies</span>
                            <span class="safe">Play Safe</span>
                        </div>
                    </div>
                    <div class="best-casinos__item">
                        <div class="best-casinos__img">
                            <img src="images/img-test/304x201.jpg" alt="test">
                            <div class="place">1</div>
                            <div class="rate">
                                <svg width="18" height="17" viewBox="0 0 18 17" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16.7363 5.89649L11.7774 5.17579L9.56055 0.681648C9.50001 0.558601 9.4004 0.458992 9.27735 0.398445C8.96876 0.246101 8.59376 0.373054 8.43946 0.681648L6.22266 5.17579L1.26368 5.89649C1.12696 5.91602 1.00196 5.98047 0.906256 6.07813C0.790556 6.19705 0.7268 6.35704 0.728998 6.52294C0.731195 6.68884 0.799166 6.84708 0.917975 6.9629L4.50587 10.4609L3.65821 15.4004C3.63833 15.5153 3.65105 15.6335 3.69491 15.7415C3.73878 15.8496 3.81204 15.9431 3.90639 16.0117C4.00074 16.0802 4.1124 16.1209 4.22872 16.1292C4.34503 16.1375 4.46134 16.113 4.56446 16.0586L9.00001 13.7266L13.4356 16.0586C13.5566 16.1231 13.6973 16.1445 13.832 16.1211C14.1719 16.0625 14.4004 15.7402 14.3418 15.4004L13.4941 10.4609L17.082 6.9629C17.1797 6.86719 17.2441 6.74219 17.2637 6.60547C17.3164 6.26368 17.0781 5.94727 16.7363 5.89649Z"
                                          fill="#F4D21F"/>
                                </svg>
                                4.3
                            </div>
                            <a href="#" class="info">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M11 7H9V5H11V7ZM11 15H9V9H11V15ZM10 0C8.68678 0 7.38641 0.258658 6.17316 0.761205C4.9599 1.26375 3.85751 2.00035 2.92892 2.92893C1.05355 4.8043 -1.52588e-05 7.34784 -1.52588e-05 10C-1.52588e-05 12.6522 1.05355 15.1957 2.92892 17.0711C3.85751 17.9997 4.9599 18.7363 6.17316 19.2388C7.38641 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.68678 19.7414 7.38642 19.2388 6.17317C18.7363 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0Z"
                                          fill="#F4D21F"/>
                                </svg>
                            </a>
                            <div class="country">
                                <img src="images/img-test/russia.svg" alt="Russia">
                                <img src="images/img-test/eng.svg" alt="England">
                            </div>
                        </div>
                        <div class="best-casinos__title-item">
                            100% up to €100 + 150FS
                        </div>
                        <a href="#" class="best-casinos__permalink white-button">
                            Play now
                        </a>
                        <div class="best-casinos__content-item">
                            <span class="age">18+</span>
                            <span class="applies">T&C applies</span>
                            <span class="safe">Play Safe</span>
                        </div>
                    </div>
                    <div class="best-casinos__item">
                        <div class="best-casinos__img">
                            <img src="images/img-test/304x201.jpg" alt="test">
                            <div class="place">1</div>
                            <div class="rate">
                                <svg width="18" height="17" viewBox="0 0 18 17" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16.7363 5.89649L11.7774 5.17579L9.56055 0.681648C9.50001 0.558601 9.4004 0.458992 9.27735 0.398445C8.96876 0.246101 8.59376 0.373054 8.43946 0.681648L6.22266 5.17579L1.26368 5.89649C1.12696 5.91602 1.00196 5.98047 0.906256 6.07813C0.790556 6.19705 0.7268 6.35704 0.728998 6.52294C0.731195 6.68884 0.799166 6.84708 0.917975 6.9629L4.50587 10.4609L3.65821 15.4004C3.63833 15.5153 3.65105 15.6335 3.69491 15.7415C3.73878 15.8496 3.81204 15.9431 3.90639 16.0117C4.00074 16.0802 4.1124 16.1209 4.22872 16.1292C4.34503 16.1375 4.46134 16.113 4.56446 16.0586L9.00001 13.7266L13.4356 16.0586C13.5566 16.1231 13.6973 16.1445 13.832 16.1211C14.1719 16.0625 14.4004 15.7402 14.3418 15.4004L13.4941 10.4609L17.082 6.9629C17.1797 6.86719 17.2441 6.74219 17.2637 6.60547C17.3164 6.26368 17.0781 5.94727 16.7363 5.89649Z"
                                          fill="#F4D21F"/>
                                </svg>
                                4.3
                            </div>
                            <a href="#" class="info">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M11 7H9V5H11V7ZM11 15H9V9H11V15ZM10 0C8.68678 0 7.38641 0.258658 6.17316 0.761205C4.9599 1.26375 3.85751 2.00035 2.92892 2.92893C1.05355 4.8043 -1.52588e-05 7.34784 -1.52588e-05 10C-1.52588e-05 12.6522 1.05355 15.1957 2.92892 17.0711C3.85751 17.9997 4.9599 18.7363 6.17316 19.2388C7.38641 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.68678 19.7414 7.38642 19.2388 6.17317C18.7363 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0Z"
                                          fill="#F4D21F"/>
                                </svg>
                            </a>
                            <div class="country">
                                <img src="images/img-test/russia.svg" alt="Russia">
                                <img src="images/img-test/eng.svg" alt="England">
                            </div>
                        </div>
                        <div class="best-casinos__title-item">
                            100% up to €100 + 150FS
                        </div>
                        <a href="#" class="best-casinos__permalink white-button">
                            Play now
                        </a>
                        <div class="best-casinos__content-item">
                            <span class="age">18+</span>
                            <span class="applies">T&C applies</span>
                            <span class="safe">Play Safe</span>
                        </div>
                    </div>
                    <div class="best-casinos__item">
                        <div class="best-casinos__img">
                            <img src="images/img-test/304x201.jpg" alt="test">
                            <div class="place">1</div>
                            <div class="rate">
                                <svg width="18" height="17" viewBox="0 0 18 17" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16.7363 5.89649L11.7774 5.17579L9.56055 0.681648C9.50001 0.558601 9.4004 0.458992 9.27735 0.398445C8.96876 0.246101 8.59376 0.373054 8.43946 0.681648L6.22266 5.17579L1.26368 5.89649C1.12696 5.91602 1.00196 5.98047 0.906256 6.07813C0.790556 6.19705 0.7268 6.35704 0.728998 6.52294C0.731195 6.68884 0.799166 6.84708 0.917975 6.9629L4.50587 10.4609L3.65821 15.4004C3.63833 15.5153 3.65105 15.6335 3.69491 15.7415C3.73878 15.8496 3.81204 15.9431 3.90639 16.0117C4.00074 16.0802 4.1124 16.1209 4.22872 16.1292C4.34503 16.1375 4.46134 16.113 4.56446 16.0586L9.00001 13.7266L13.4356 16.0586C13.5566 16.1231 13.6973 16.1445 13.832 16.1211C14.1719 16.0625 14.4004 15.7402 14.3418 15.4004L13.4941 10.4609L17.082 6.9629C17.1797 6.86719 17.2441 6.74219 17.2637 6.60547C17.3164 6.26368 17.0781 5.94727 16.7363 5.89649Z"
                                          fill="#F4D21F"/>
                                </svg>
                                4.3
                            </div>
                            <a href="#" class="info">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M11 7H9V5H11V7ZM11 15H9V9H11V15ZM10 0C8.68678 0 7.38641 0.258658 6.17316 0.761205C4.9599 1.26375 3.85751 2.00035 2.92892 2.92893C1.05355 4.8043 -1.52588e-05 7.34784 -1.52588e-05 10C-1.52588e-05 12.6522 1.05355 15.1957 2.92892 17.0711C3.85751 17.9997 4.9599 18.7363 6.17316 19.2388C7.38641 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.68678 19.7414 7.38642 19.2388 6.17317C18.7363 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0Z"
                                          fill="#F4D21F"/>
                                </svg>
                            </a>
                            <div class="country">
                                <img src="images/img-test/russia.svg" alt="Russia">
                                <img src="images/img-test/eng.svg" alt="England">
                            </div>
                        </div>
                        <div class="best-casinos__title-item">
                            100% up to €100 + 150FS
                        </div>
                        <a href="#" class="best-casinos__permalink white-button">
                            Play now
                        </a>
                        <div class="best-casinos__content-item">
                            <span class="age">18+</span>
                            <span class="applies">T&C applies</span>
                            <span class="safe">Play Safe</span>
                        </div>
                    </div>
                    <div class="best-casinos__item">
                        <div class="best-casinos__img">
                            <img src="images/img-test/304x201.jpg" alt="test">
                            <div class="place">1</div>
                            <div class="rate">
                                <svg width="18" height="17" viewBox="0 0 18 17" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16.7363 5.89649L11.7774 5.17579L9.56055 0.681648C9.50001 0.558601 9.4004 0.458992 9.27735 0.398445C8.96876 0.246101 8.59376 0.373054 8.43946 0.681648L6.22266 5.17579L1.26368 5.89649C1.12696 5.91602 1.00196 5.98047 0.906256 6.07813C0.790556 6.19705 0.7268 6.35704 0.728998 6.52294C0.731195 6.68884 0.799166 6.84708 0.917975 6.9629L4.50587 10.4609L3.65821 15.4004C3.63833 15.5153 3.65105 15.6335 3.69491 15.7415C3.73878 15.8496 3.81204 15.9431 3.90639 16.0117C4.00074 16.0802 4.1124 16.1209 4.22872 16.1292C4.34503 16.1375 4.46134 16.113 4.56446 16.0586L9.00001 13.7266L13.4356 16.0586C13.5566 16.1231 13.6973 16.1445 13.832 16.1211C14.1719 16.0625 14.4004 15.7402 14.3418 15.4004L13.4941 10.4609L17.082 6.9629C17.1797 6.86719 17.2441 6.74219 17.2637 6.60547C17.3164 6.26368 17.0781 5.94727 16.7363 5.89649Z"
                                          fill="#F4D21F"/>
                                </svg>
                                4.3
                            </div>
                            <a href="#" class="info">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path d="M11 7H9V5H11V7ZM11 15H9V9H11V15ZM10 0C8.68678 0 7.38641 0.258658 6.17316 0.761205C4.9599 1.26375 3.85751 2.00035 2.92892 2.92893C1.05355 4.8043 -1.52588e-05 7.34784 -1.52588e-05 10C-1.52588e-05 12.6522 1.05355 15.1957 2.92892 17.0711C3.85751 17.9997 4.9599 18.7363 6.17316 19.2388C7.38641 19.7413 8.68678 20 10 20C12.6522 20 15.1957 18.9464 17.0711 17.0711C18.9464 15.1957 20 12.6522 20 10C20 8.68678 19.7414 7.38642 19.2388 6.17317C18.7363 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0Z"
                                          fill="#F4D21F"/>
                                </svg>
                            </a>
                            <div class="country">
                                <img src="images/img-test/russia.svg" alt="Russia">
                                <img src="images/img-test/eng.svg" alt="England">
                            </div>
                        </div>
                        <div class="best-casinos__title-item">
                            100% up to €100 + 150FS
                        </div>
                        <a href="#" class="best-casinos__permalink white-button">
                            Play now
                        </a>
                        <div class="best-casinos__content-item">
                            <span class="age">18+</span>
                            <span class="applies">T&C applies</span>
                            <span class="safe">Play Safe</span>
                        </div>
                    </div>
                </div>
                <a href="#" class="best-casinos__lnk red-btn">
                    Show more
                    <svg width="17" height="8" viewBox="0 0 17 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd"
                              d="M13.5447 0.716759L16.7267 3.89874C16.922 4.094 16.922 4.41058 16.7267 4.60585L13.5447 7.78783C13.3495 7.98309 13.0329 7.98309 12.8376 7.78783C12.6424 7.59256 12.6424 7.27598 12.8376 7.08072L15.1661 4.75229H0.373169V3.75229H15.1661L12.8376 1.42387C12.6424 1.2286 12.6424 0.912021 12.8376 0.716759C13.0329 0.521497 13.3495 0.521497 13.5447 0.716759Z"
                              fill="#F4D21F"/>
                    </svg>
                </a>
            </div>
        </section>
        <section class="newonline">
            <div class="newonline__container container-wide">
                <div class="newonline__slots">
                    <h2 class="newonline__slots-title">
                        New Online Slots Announced
                    </h2>
                    <div class="newonline__slots-list">
                        <a href="#" class="newonline__slots-item">
                            <div class="newonline__slots-img">
                                <picture>
                                    <source srcset="images/img-test/304_201_2.jpg" media="(max-width: 600px)">
                                    <img src="images/img-test/304_201_2.jpg" alt="">
                                </picture>
                                <div class="newonline__slots-date">
                                    May 2021
                                </div>
                            </div>
                            <div class="newonline__slots-content">
                                <h3 class="newonline__slots-title single-item-title">
                                    Jaguar SuperWays
                                </h3>
                                <div class="newonline__slots-subtitle single-item-desc">
                                    ReelPlay
                                </div>
                            </div>
                        </a>
                        <a href="#" class="newonline__slots-item">
                            <div class="newonline__slots-img">
                                <picture>
                                    <source srcset="images/img-test/304_201_2.jpg" media="(max-width: 600px)">
                                    <img src="images/img-test/304_201_2.jpg" alt="">
                                </picture>
                                <div class="newonline__slots-date">
                                    May 2021
                                </div>
                            </div>
                            <div class="newonline__slots-content">
                                <h3 class="newonline__slots-title single-item-title">
                                    Jaguar SuperWays
                                </h3>
                                <div class="newonline__slots-subtitle single-item-desc">
                                    ReelPlay
                                </div>
                            </div>
                        </a>
                        <a href="#" class="newonline__slots-item">
                            <div class="newonline__slots-img">
                                <picture>
                                    <source srcset="images/img-test/304_201_2.jpg" media="(max-width: 600px)">
                                    <img src="images/img-test/304_201_2.jpg" alt="">
                                </picture>
                                <div class="newonline__slots-date">
                                    May 2021
                                </div>
                            </div>
                            <div class="newonline__slots-content">
                                <h3 class="newonline__slots-title single-item-title">
                                    Jaguar SuperWays
                                </h3>
                                <div class="newonline__slots-subtitle single-item-desc">
                                    ReelPlay
                                </div>
                            </div>
                        </a>
                        <a href="#" class="newonline__slots-item">
                            <div class="newonline__slots-img">
                                <picture>
                                    <source srcset="images/img-test/304_201_2.jpg" media="(max-width: 600px)">
                                    <img src="images/img-test/304_201_2.jpg" alt="">
                                </picture>
                                <div class="newonline__slots-date">
                                    May 2021
                                </div>
                            </div>
                            <div class="newonline__slots-content">
                                <h3 class="newonline__slots-title single-item-title">
                                    Jaguar SuperWays
                                </h3>
                                <div class="newonline__slots-subtitle single-item-desc">
                                    ReelPlay
                                </div>
                            </div>
                        </a>
                        <a href="#" class="newonline__slots-item">
                            <div class="newonline__slots-img">
                                <picture>
                                    <source srcset="images/img-test/304_201_2.jpg" media="(max-width: 600px)">
                                    <img src="images/img-test/304_201_2.jpg" alt="">
                                </picture>
                                <div class="newonline__slots-date">
                                    May 2021
                                </div>
                            </div>
                            <div class="newonline__slots-content">
                                <h3 class="newonline__slots-title single-item-title">
                                    Jaguar SuperWays
                                </h3>
                                <div class="newonline__slots-subtitle single-item-desc">
                                    ReelPlay
                                </div>
                            </div>
                        </a>
                    </div>
                    <a href="#" class="newonline__slots-lnk red-btn">
                        2500+ Slots
                        <svg width="17" height="9" viewBox="0 0 17 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M13.1716 0.96444L16.3536 4.14642C16.5488 4.34168 16.5488 4.65826 16.3536 4.85353L13.1716 8.03551C12.9763 8.23077 12.6597 8.23077 12.4645 8.03551C12.2692 7.84025 12.2692 7.52366 12.4645 7.3284L14.7929 4.99997H0V3.99997H14.7929L12.4645 1.67155C12.2692 1.47628 12.2692 1.1597 12.4645 0.96444C12.6597 0.769178 12.9763 0.769178 13.1716 0.96444Z" fill="#F4D21F"></path>
                        </svg>
                    </a>
                </div>
            </div>
        </section>
    </div>
<?php
include("template-parts/footer.php");
?>